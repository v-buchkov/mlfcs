...
class Volatility:
    def __init__(
        ...
        assert returns is not None or volatilities is not None, (
            "Either returns or volatilities must be provided!"
        )

        assert returns is None or volatilities is None, (
            "Only one of returns or volatilities must be provided!"
        )
        ...
        self.method = (
            ...
            else VolatilityMethod.SAMPLE_VOL
        )
        self.volatilities = (
            self._calculate_volatilities() if volatilities is None else volatilities
        )
