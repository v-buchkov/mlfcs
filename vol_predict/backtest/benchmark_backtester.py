...
class BacktestResults:
    def __init__(
        self,
        model,
        last_train_date: datetime = datetime.strptime(
            "2018-07-01 00:00:00", "%Y-%m-%d %H:%M:%S"
        ),
        ...
        if forecasts.shape[0] != true_vola.shape[0]:
            raise ValueError(
                "The number of rows in the forecasts and true volatility must be the same."
            )

        self.true_vola = true_vola
        self.residuals = (
            self.true_vola - forecasts
        )  # nans for timestamps where forecasts are missing
...
class BenchmarkBacktester:
    ...
    def __init__(
        ...
        last_train_date: datetime = datetime.strptime(
            "2018-06-30 23:59:59", "%Y-%m-%d %H:%M:%S"
        ),
        is_training_expanded: bool = True,
        forecast_horizon: int = 1,
        ...
        **kwargs,
        ...
        self.bias_lookback = pd.Timedelta(
            hours=24
        )  # used to de-bias prediction if modelling log(y)

    def backtest(
        self, benchmark, hyperparams=None, use_ob_feats=True, use_log_y=True
    ) -> BacktestResults:
        """
        TODO add docstring
        """
        ...
        if hyperparams is None:
            ...
            model = benchmark(
                **hyperparams, use_ob_feats=use_ob_feats, use_log_y=use_log_y
            )

        if not hasattr(model, "label_name"):
            model.label_name = "log_vol" if use_log_y else "vol"

        output = self.output_template.copy().rename(columns={"model_name": model.name})
        ...
        for t in tqdm(output.index):
            ...
            if output.loc[t, "retraining_flag"]:
                ...
                if self.is_training_expanded:
                    ...
                    model.fit(
                        y=self.dataset.loc[
                            : t - pd.Timedelta(hours=1), model.label_name
                        ],
                        X=self.dataset.loc[
                            : t - pd.Timedelta(hours=1), model.feature_names
                        ],
                    )
                else:
                    ...
                    model.fit(
                        y=self.dataset.loc[
                            t - self.lookback : t - pd.Timedelta(hours=1),
                            model.label_name,
                        ],
                        X=self.dataset.loc[
                            t - self.lookback : t - pd.Timedelta(hours=1),
            ...
            pred = model.forecast(
                steps=self.forecast_horizon, X=self.dataset.loc[:t, model.feature_names]
            )

            if use_log_y:
                conditional_log_vola = np.var(
                    self.dataset.loc[
            ...
            model.update(
                new_y=self.dataset.loc[t:t, model.label_name],
