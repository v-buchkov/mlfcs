...
#plot_acf_pacf(log_open)

#Determines the d using the Augmented Dickey–Fuller test 
...
d = determine_d(train, 0.01, 5)

#plot_acf_pacf(log_open.diff().dropna())
...
def plot_resids_acf_pacf(model):
    resids = model.resid[1:]
    fig, ax = plt.subplots(1,2)
    sm.graphics.tsa.plot_acf(resids, ax=ax[0])
    sm.graphics.tsa.plot_pacf(resids, ax=ax[1])
    plt.show()
